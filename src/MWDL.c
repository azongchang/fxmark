...
/**
 * Nanobenchmark: mkdir load (private dir)
 *   Each worker creates directories under its private root.
 */
#include <sys/types.h>
#include <sys/stat.h>
#include <fcntl.h>
...
static int main_work(struct worker *worker)
{
	...
	for (iter = 0; !bench->stop; ++iter) {
		char path[PATH_MAX];
		snprintf(path, PATH_MAX, "%s/n_mkdir-%" PRIu64, test_root, iter);
...
struct bench_operations n_mkdir_ops = {
	.pre_work  = pre_work,
	.main_work = main_work,
};
