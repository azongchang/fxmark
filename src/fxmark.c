...
static struct bench_desc bench_table[] = {
	...
	{ "MWRM",
	  ...
	  &n_dir_ins_ops },
	{ "DWSL", "journal commit: each process fsync a private file",
	  &n_jnl_cmt_ops },
	{ "DWOM",
	...
	{ "MWRL",
	  ...
	  &n_file_rename_ops },
	{ "DRBL", "file read: each process read a block of its private file",
	  &n_file_rd_ops },
	{ "DRBL_bg", "file read with a background writer", &n_file_rd_ops },
	{ "DRBM",
	  "shared file read: each process reads its private region of the shared file",
	  &n_shfile_rd_ops },
	{ "DRBM_bg", "shared file read with a background writer",
	  &n_shfile_rd_bg_ops },
	{ "DRBH",
	  "shared blk read: each process reads the same page of the shared file",
	  &n_shblk_rd_ops },
	{ "DRBH_bg", "shared blk read with a background writer",
	  &n_shblk_rd_bg_ops },
	{ "MRDL",
	  "directory read: each process reads entries of its private directory",
	  &n_dir_rd_ops },
	{ "MRDL_bg", "directory read with a background writer",
	  &n_dir_rd_bg_ops },
	{ "MRDM",
	  "shared directory read: each process reads entries of the shared directory",
	  &n_shdir_rd_ops },
	{ "MRDM_bg", "shared directory read with a background writer",
	  &n_shdir_rd_bg_ops },
	{ "MRPL", "path resolution for a private file", &n_priv_path_rsl_ops },
	{ "MRPM",
	  "path resolution: each process does stat() at random files in 8-level directories with 8-branching-out factor",
	  &n_path_rsl_ops },
	{ "MRPM_bg", "path resolution  with a background writer",
	  &n_path_rsl_bg_ops },
	{ "MRPH", "path resolution at the same level directory",
	  &n_spath_rsl_ops },
	{ "MWDL",
	  "mkdir load: each process creates directories in its private directory",
	  &n_mkdir_ops },
	{ "MWDM",
	  "mkdir load: each process creates directories at the test root",
	  &u_mkdir_ops },
	{ "MWNL",
	  "mknod load: each process creates special files in its private directory",
	  &n_mknod_ops },
	{ "MWNM",
	  "mknod load: each process creates special files at the test root",
	  &u_mknod_ops },
	{ "MWCM", "each process creates files in their private directory",
	  &u_file_cr_ops },
	{ "MWUL", "each process deletes files in their private directory",
	  &u_file_rm_ops },
	{ "MWUM", "each process deletes files at the test root directory",
	  &u_sh_file_rm_ops },
	{ "DWTL",
	  "each process truncates its private file at the test root directory",
	  &u_file_tr_ops },
	{ "MWRH", "each process renames files at the test root directory",
	  &n_rename_ops },
	{ NULL, NULL, NULL },
};
